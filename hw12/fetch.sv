...
logic [10:0] pc_in, pc_out, mar_q, stack_out, w_change, k_change;
logic load_pc, load_mar, load_ir, load_w, movlw, addlw, iorlw, sublw, 
	  andlw, xorlw, sel_alu, ram_en, dir, addwf, andwf, clrf, 
	  ...
	  asrf, lslf, lsrf, rlf, rrf, swapf, push, pop, call, RETURN, bra, brw, nop, rst_ir;
logic [4:0] op;
logic [7:0] alu, mux1_out, ram_out, databus, ram_mux, bcf_mux, bsf_mux, btfsc_skip_bit, btfss_skip_bit, w_q;
logic [2:0] sel_ram_mux, sel_bit, sel_pc;
...
stack s0(
	.stack_out(stack_out), .stack_in(pc_in), .push(push), .pop(pop), .rst(rst), .clk(clk)
);
...
always_ff@(posedge clk)
begin
	if(rst_ir) ir_q <= 14'b00000000000000;
...
begin
	case(sel_pc)
		...
		1: pc_out = ir_q[10:0];
		2: pc_out = stack_out;
		3: pc_out = pc_in + k_change;
		4: pc_out = pc_in + w_change;
	endcase
end
...
assign RETURN = ir_q[13:0] == 13'h0008;

assign bra = ir_q[13:9] == 5'b11001;
assign brw = ir_q[13:0] == 13'h000b;
assign nop = ir_q[13:0] == 13'b0;
...
begin
	...
	sel_ram_mux = 0; push = 0; pop = 0; rst_ir = 0;
	case(ps)
		T0:
		begin
			ns = T1;
